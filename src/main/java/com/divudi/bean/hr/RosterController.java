...
public class RosterController implements Serializable {
    ...
    public List<Roster> getSelectedItems() {
        selectedItems = getFacade().findBySQL("select c from Roster c where c.retired=false and upper(c.name) like '%" + getSelectText().toUpperCase() + "%' order by c.name");
    ...
    public List<Roster> completeRoster(String qry) {
        List<Roster> a = null;
        if (qry != null) {
            a = getFacade().findBySQL("select c from Roster c where c.retired=false and upper(c.name) like '%" + qry.toUpperCase() + "%' order by c.name");
        }
        if (a == null) {
            a = new ArrayList<Roster>();
        }
        return a;
    }
